...
public class StateUtils {
    ...
    public static @Nullable DeviceType determineTargetTypeSemantic(ConfigStore cs, Item item,
        ...
        boolean isLightSource = item.getGroupNames().stream().map(itemRegistry::get).filter(Objects::nonNull)
                .filter(GroupItem.class::isInstance).map(GroupItem.class::cast)
                .anyMatch(SemanticsPredicates.isA(lightSourceTag));

        return isLightSource ? deviceType : null;
    }

    public static @Nullable GroupItem getSemanticEquipmentItem(Item item, ItemRegistry itemRegistry) {
        Class<? extends Tag> lightSourceTag = SemanticTags.getById(Equipment.LIGHT_SOURCE.getUID());
        if (lightSourceTag == null) {
            return null;
        }
        ...
        return item.getGroupNames().stream().map(itemRegistry::get).filter(Objects::nonNull)
                .filter(GroupItem.class::isInstance).map(GroupItem.class::cast)
                .filter(SemanticsPredicates.isA(lightSourceTag)).findFirst().orElse(null);
    }

    public static boolean isSemanticLocationItem(Item item, ItemRegistry itemRegistry) {
